...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑新片预告</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>新片预告管理</span></li>
								<li class="breadcrumb-item active"><span>编辑新片预告</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">新片预告信息</h3>
									...
									<div class="form-group col-md-6">
										...
										<input name="dianyingyugao" id="dianyingyugao-input" type="hidden">
											...
											<div class="form-group  col-md-12">
												<label>电影介绍</label>
												<div><div class="upload"><input id="dianyingjieshaoupload" name="file" type="file">选择文件</div>
												<script id="dianyingjieshaoEditor" type="text/plain" style="width:800px;height:230px;"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var dianyingjieshaoUe = UE.getEditor('dianyingjieshaoEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="dianyingjieshao" id="dianyingjieshao-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "xinpianyugao/info/" + id,
		...
		function upload(){
			...
			$('#dianyingjieshaoupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('dianyingjieshaoEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("xinpianyugao/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.dianyingjieshao != null && ruleForm.dianyingjieshao != 'null' && ruleForm.dianyingjieshao != ''){
				var dianyingjieshaoMes = '' + ruleForm.dianyingjieshao;
				var dianyingjieshaoUeditor = UE.getEditor('dianyingjieshaoEditor');
				dianyingjieshaoUeditor.ready(function() {
					dianyingjieshaoUeditor.setContent(dianyingjieshaoMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('dianyingjieshaoEditor').hasContents()){
				$('#dianyingjieshao-input').attr('value',UE.getEditor('dianyingjieshaoEditor').getPlainTxt());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("xinpianyugao/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#dianyingjieshao").val(ruleForm.dianyingjieshao);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.dianyingjieshao != null && res.data.dianyingjieshao != ''){

					$("#dianyingjieshao").val(res.data.dianyingjieshao);
					}
