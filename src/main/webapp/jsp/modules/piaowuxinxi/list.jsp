...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">票务信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>票务信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">票务信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="dianyingleixing" id="dianyingleixingSelect" class="form-control form-control-sm" aria-controls="tableId">
										<option value=" "> </option>
										<option value="3D电影">3D电影</option>
										<option value="2D电影">2D电影</option>
									</select>
									<label>
										放映厅
									</label>
									<select name="fangyingting" id="fangyingtingSelect" class="form-control form-control-sm" aria-controls="tableId">
									</select>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('shangyingshijian')">上映时间<i id="shangyingshijianIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('fangyingshijian')">放映时间<i id="fangyingshijianIcon" class="fa fa-sort"></i></th>

											<th>电影预告</th>
											<th onclick="sort('yingyuandizhi')">影院地址<i id="yingyuandizhiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('goupiaoxuzhi')">购票须知<i id="goupiaoxuzhiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var fangyingtingIndex = document.getElementById("fangyingtingSelect").selectedIndex;
			var fangyingtingOptions = document.getElementById("fangyingtingSelect").options;
			var fangyingtingSelectedOption = fangyingtingOptions[fangyingtingIndex]
			if(fangyingtingSelectedOption.text != null && fangyingtingSelectedOption.text != ''){
				searchForm.fangyingting = "%" + fangyingtingSelectedOption.text + "%";
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("piaowuxinxi/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				...
				fangyingting : searchForm.fangyingting,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var shangyingshijianCell  = document.createElement('td');

						 shangyingshijianCell.innerHTML = item.shangyingshijian;


			row.appendChild(shangyingshijianCell);
			...
			var fangyingshijianCell  = document.createElement('td');

						  fangyingshijianCell.innerHTML = item.fangyingshijian; 

			row.appendChild(fangyingshijianCell);
			var dianyingyugaoCell  = document.createElement('td');

						  var dianyingyugaoMedia = document.createElement('button');
			  dianyingyugaoMedia.setAttribute('class','btn-table')
			  dianyingyugaoMedia.innerHTML = '播放';
			  var dianyingyugaoAttr = "mediaPlay('" + item.dianyingyugao + "')"
			  dianyingyugaoMedia.setAttribute('onclick',dianyingyugaoAttr);
       		          dianyingyugaoCell.appendChild(dianyingyugaoMedia);


			row.appendChild(dianyingyugaoCell);
			...
			var goupiaoxuzhiCell  = document.createElement('td');

						 goupiaoxuzhiCell.innerHTML = item.goupiaoxuzhi;


			row.appendChild(goupiaoxuzhiCell);
            ...
            crossBtn1.innerHTML = "购票"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"dingpiaoxinxi\")");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("piaowuxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"piaowuxinxi");
        ...
        function pay(id){
			...
			window.sessionStorage.setItem('tableName',"piaowuxinxi");
